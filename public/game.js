...
class Player {
  constructor(game, options) {
    ...
    loader.load(`${game.assetsPath}fbx/people/${model}.fbx`, function (object) {
      ...
      textureLoader.load(
        `${game.assetsPath}images/body.png`,
        function (texture) {
          object.traverse(function (child) {
            if (child.isMesh) {
      ...
      player.object.position.set(-100, 20, -1060);
  ...
  constructor(game, model) {
    ...
    $("#m").on("keydown", function (e) {
      if (e.keyCode === 13 && !e.shiftKey) {
        ...
        e.preventDefault(); // prevent default Enter key behavior (which is adding a new line)
        $("#msg-form").submit(); // manually trigger form submission
      }
    });
